...
def wait_check(proc_pid_arr: list, manager: procManager):
    ...
    if exit_code != 0:
        print('Something bad happens! exit_code: {}'.format(exit_code))
    ...
    if state != proc_state.NORMAL:
        print('Child {} is in abnormal state {}!'.format(idx, proc_state.tpe[state]))
        for pid in proc_pid_arr:
            if pid != end_pid:
                os.kill(pid, signal.SIGKILL)
        exit(-1)
...
if not multicore:
    ...
    print('Muticore Fuzzing, Number of batches -- {}'.format(num_batch))
    ...
    for i in range(num_batch * multicore):
        ...
        print('Multicore Fuzzing Batch [{}], Thread [{}]'.format(i, proc_num))
